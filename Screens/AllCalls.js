...
const AllCalls = () => {
    ...
    const handleCallLog = async (callLog) => {
        try {
            await Promise.all(
                callLog?.map(async (callLogs, key) => {
                    const logDate = callLogs.dateTime;
                    const requestData = {
                        user_id: isAuthenticated,
                        name: callLogs.name,
                        phone_number: callLogs.phoneNumber,
                        type: callLogs.type,
                        datetime: callLogs.dateTime,
                        duration: callLogs.duration,
                        rawtype: callLogs.rawType,
                    };
                    // console.log('Response Content:', requestData);

                    if (isAuthenticated) {
                        const response = await fetch(`${PROCESS_KEY}/add_call_log`, {
                            method: 'POST',
                            headers: {
                        ...
                        console.error('User ID not available. API call skipped.');
                    }
            ...
            <View style={styles.callItem}>
                <View style={styles.callDetails}>
                    ...
                    <View style={styles.rightSection}>
                        ...
                        {/* <Text style={styles.number}>{splitArray['0']}</Text> */}
